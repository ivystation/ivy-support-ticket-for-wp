...
## 13. SSO 아키텍처 (v0.2.0 변경)

### 13.1 배경

초기 설계(§5)에서는 모든 티켓 UI를 WP 어드민 내부에 직접 구현했다. 그러나 ticket.ivynet.co.kr이 독립적인 Next.js 앱으로 성숙하면서, UI를 두 곳에서 유지보수하는 비용이 발생했다. 이를 해결하기 위해 **새 창 열기 + SSO** 방식으로 전환했다.

### 13.2 선택된 방식: 새 창 + SSO

WordPress 메뉴 클릭 → HS256 JWT 생성 → ticket.ivynet.co.kr을 새 탭으로 열고 자동 로그인

```
WP 플러그인                ticket.ivynet.co.kr        pm.ivynet.co.kr
     │                            │                         │
     │ window.open(SSO URL)       │                         │
     │──────────────────────────▶│                         │
...
| 항목 | 방식 1: 새 창 (선택) | 방식 2: Fullwidth 모달 + iframe |
|------|----------------------|--------------------------------|
| 플러그인 코드량 | 최소 (~80줄 SSO) | 중간 (모달 CSS/JS 추가) |
| 안정성 | 높음 (iframe 없음) | 낮음 (X-Frame-Options/CSP 관리 필요) |
| 파일 다운로드 | 완전 지원 | 제한 있음 |
| 유지보수 | 낮음 | ticket CSP 설정 지속 관리 필요 |

### 13.4 JWT 토큰 구조

WP 플러그인이 `hash_hmac('sha256', ...)` + base64url 인코딩으로 표준 HS256 JWT를 생성한다.
...
- 만료: 5분 (`exp = iat + 300`)
- 시크릿: WP `sso_secret` 설정값 = pm `SSO_SECRET` 환경변수

### 13.5 하위 호환

SSO 미설정(`sso_secret` 또는 `ticket_url` 미입력) 시 기존 내장 UI(티켓 목록·작성·상세)가 그대로 동작한다. 설정 완료 시에만 SSO 런처로 전환된다.
...
| 일자 | 버전 | 변경 사항 |
|---|---|---|
| 2026-05-04 | 0.2.0 | SSO 아키텍처 전환 — ticket.ivynet.co.kr 새 탭 자동 로그인. 설정 SSO 탭 신설. §13 추가. |
